...
for (p in phase) {
  for (d in day) {
    tmpTable <- list.files(path = sprintf("/Volumes/CAP/data/rawData/phase%d/day%d/riskyChoice", p, d), pattern = "*.csv", full.names = TRUE) %>%    # Get all the csv files in specified folder
    ...
    tmpVec =list.files(path = sprintf("/Volumes/CAP/data/rawData/phase%d/day%d/riskyChoice", p, d), pattern = "csv$");
    
    if (length(unique(tmpTable$participant)) == length(tmpVec)){
      file2save = sprintf("/Volumes/CAP/data/rawData/combinedRawData/RDMday/collateRDMphase%dday%d.csv",p,d,p,d)
