...
# Keep the columns we need and rename to subLevelLong because this is the long format dataframe (each participant and phase gets a row)
subLevelLong = qualtricsBothPhs[names(qualtricsBothPhs) %in% keepCols]; #keep only the columns we need.
...
wideColNames = c("phs1_pss_stressedToday", 
...
sesPhs1pca$x[,'PC1'] # 520 rows
sesPhs2pca$x[,'PC1'] # 347 rows
...
```{r}
#rdataDir=config$path$Rdata

save(file= subLevelLong.Rdata",subLevelLong);
save(file="subLevelWide.Rdata",subLevelWide);
```
