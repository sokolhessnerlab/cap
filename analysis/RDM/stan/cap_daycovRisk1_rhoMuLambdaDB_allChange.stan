...
transformed parameters {
  ...
  for(t in 1:N) { // for each trial
    ...
    ltmp[t] = exp(l[ind[t]] + day[t] * dayLambda +
                              covRisk[t] * covRiskLambda + 
                              day[t] * covRisk[t] * dayxcovRiskLambda);
    mtmp[t] = exp(m[ind[t]] + day[t] * dayMu +
                              covRisk[t] * covRiskMu + 
                              day[t] * covRisk[t] * dayxcovRiskMu);
...
model {
  ...
  dayLambda ~ normal(0,5); // generous psychologically plausible range for changes in Lambda given exp() transformation
  covRiskLambda ~ normal(0,5);
  dayxcovRiskLambda ~ normal(0,5);

  dayMu ~ normal(0,5); // generous psychologically plausible range for changes in Mu given exp() transformation
  covRiskMu ~ normal(0,5);
  dayxcovRiskMu ~ normal(0,5);
