...
couple of ways to speed things up:
1) try running on lab computer
2) are fixed effects models doing ok (are they producing similar results and running much faster)? If so, we *may* be able to use this as a way to do faster modeling (iterations) and then following up with full mixed regressions.
```{r}
...
# CHECK - NUMBER OF OBSERVATIONS MAY BE DIFFERENT HERE BECAUSE DATA IS DIFFERENT
