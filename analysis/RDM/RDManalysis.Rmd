...
# singular because random effects have very small variance when using GLMER - use GLM instead (beta and pvalue are exactly the same in glmer and glm)
...
# 1) resrict data to one phase and if the pattern holds up, we atleast know that individual differences do matter
  # using this model to see becauase concern about using offset and putting covid risk in second-step model is making it hard to see effects: rdmChoice ~ 1 + rdmGainSC + rdmSafeSC + rdmGroundEV + covq_PAB_q1_personalRisk_scaledNoNA
  # just phase 1 data - negative and significant; just phase 2 - negative and significant 
  
  # using glmer(rdmChoice ~  0 + covq_PAB_q1_personalRisk_scaledNoNA  + (1|subID) with offset and restricting data to just one     phase, Phase 1: beta = -0.009468, p = .81 and Phase 2: beta = -0.23339, p = 0.0012
...
# interacting phaseRecode with covid risk (noNA) with no glmer, there is a small positive effect of covid risk, bigger positive effect of phase and interaction between covid risk and phase where people are basically around .5 pgam on first phase regardless of how they perceive risk but there is a big split during phase 2 where people who have high perceived covid risk in phase 2 gamble much less and those who have low perceived covid risk in phase 2 gamble much more, but people in middle stay the same.

# if we run the same model with dayoverall scaled variable, the pattern is the same as phase (but with day, we capture more data points with 40 days).

#Day is much better AIC than phase model

# variability in gambling is smaller during phase 1 than phase 2 (where its really big) and this matches with the covid risk interaction with phase and/or day.
